	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"none","width":"1200px","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('quhuoxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="取货时间" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.quhuoshijian}}
							</template>
						</el-table-column>
					</el-table>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/quhuoxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('quhuoxinxiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
