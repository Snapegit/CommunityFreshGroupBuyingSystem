		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="取货时间" prop="quhuoshijian">
							<el-date-picker
								class="list_date"
								v-model="form.quhuoshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.quhuoshijian?true:false"
								placeholder="请选择取货时间" />
						</el-form-item>
					</el-col>
				</el-row>
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.quhuoshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='quhuoshijian'){
					form.value.quhuoshijian = row[x];
					disabledForm.value.quhuoshijian = true;
					continue;
				}
			}
			if(row){
